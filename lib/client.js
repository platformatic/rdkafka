...
Client.prototype.disconnect = function(cb) {
  ...
  if (cb) {
    if (this._client) {
      ...
      // TODO(mcollina): I don't think this can even happen
      process._rawDebug('Client is already disconnected');
      cb()
    }
  }
  ...
  if (!this._isDisconnecting && this._client) {
    ...
    process._rawDebug('disconnecting client', this._client);
    this._client.disconnect(function() {
      process._rawDebug('Client actually disconnected');
